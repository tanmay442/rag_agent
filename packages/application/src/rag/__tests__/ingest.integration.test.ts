...
function makeDeps(overrides?: Partial<IngestDeps>): IngestDeps {
  ...
  return {
    documents: {
      ...
      findById: vi.fn(),
      saveBlob: vi.fn(),
      insert: vi.fn().mockResolvedValue({ id: 1, fileName: 'test.pdf', fileHash: 'abc', uploadedBy: 'user', uploadedAt: new Date(), blob: null, deletedAt: null }),
      deleteById: vi.fn(),
      softDelete: vi.fn(),
      restore: vi.fn(),
      listDeletedSince: vi.fn().mockResolvedValue([]),
      updateBlob: vi.fn(),
      list: vi.fn(),
...
describe('ingestFile', () => {
  ...
  it('deletes old document only after new insert succeeds', async () => {
    ...
    const insert = vi.fn().mockResolvedValue({ id: 2, fileName: 'test.pdf', fileHash: 'newhash', uploadedBy: 'user', uploadedAt: new Date(), blob: null, deletedAt: null });
    const deps = makeDeps({
      documents: {
        findByName: vi.fn().mockResolvedValue({ id: 1, fileName: 'test.pdf', fileHash: 'oldhash', uploadedBy: 'user', uploadedAt: new Date(), blob: null, deletedAt: null }),
        findById: vi.fn(),
        saveBlob: vi.fn(),
        insert,
        deleteById,
  ...
  it('returns unchanged when hash matches', async () => {
    const deps = makeDeps({
      documents: {
        ...makeDeps().documents,
        findByName: vi.fn().mockResolvedValue({ id: 1, fileName: 'test.pdf', fileHash: 'abc123', uploadedBy: 'user', uploadedAt: new Date(), blob: null, deletedAt: null }),
      },
    });
    const result = await ingestFile(
